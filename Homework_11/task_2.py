...
try:
    ...
    first_auth_btn = driver.find_element(By.CSS_SELECTOR, '[data-qa="auth-AdaptiveLoginForm__checkSignInTypeButton"]')
    first_auth_btn.click()
    time.sleep(1)
    pass_on_page = driver.find_element(By.CSS_SELECTOR, '[name="Password"]')
    pass_on_page.send_keys(my_pass)
    time.sleep(1)
    assert pass_on_page.get_attribute('value') == my_pass
    login_btn = driver.find_element(By.CSS_SELECTOR, '[data-qa="auth-AdaptiveLoginForm__signInButton"]')
    login_btn.click()
    time.sleep(7)
